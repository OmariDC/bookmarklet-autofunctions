(function(){
  ...
  const logUnknown = (word, sentence) => {
    ...
    if (state.flatMap[lower] || state.customSet.has(word) || state.canonicals.some(c => c.toLowerCase() === word.toLowerCase())) return;
  ...
  const replaceSlice = (root, start, end, replacement) => {
    ...
    r2.setStart(textNode, replacement.length);
    r2.collapse(true);
    sel.addRange(r2);
  };
  ...
  const processText = (root) => {
    ...
    for (const rule of state.multi) {
      const idx = lowerBefore.lastIndexOf(rule.src);
      if (idx >= 0) {
        const nextChar = before[idx + rule.src.length];
        if (!nextChar || /[\s.,!?]/.test(nextChar)) {
          const newBefore = before.slice(0, idx) + rule.tgt;
          const updated = newBefore + before.slice(idx + rule.src.length) + after;

          root.innerText = updated;

          const caretPos = updated.length - after.length;
          const pos = findTextPosition(root, caretPos);
          if (pos) {
            const sel = window.getSelection();
            sel.removeAllRanges();
